...
def run(args):
    ...
    with open(args.fname, 'r') as f:
        ...
        if args.command == 'columns':
            ...
            # print it out
            for col in args.columns:
                print_row([col, "Count"], True)
                xy_data = []
                n = 1
                for item in sorted(summary[col].items(), key=lambda x: x[1], reverse=True):
                    print_row(item)
                    xy_data.append(item)
                    n += 1
                ...
                # plot?
                if args.do_plot:
                    plt.plot([ x[0] for x in xy_data ], [ x[1] for x in xy_data ], 'bo')
                    plt.ylabel('Count')
